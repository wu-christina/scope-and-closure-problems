...
function smoothieMachine(...ingred) {
  let sentence = "I'm having a smoothie with "
  let ingredCount = 0;

  if (ingred !== undefined) {
      sentence += ingred.join(" and ");
      ingredCount = ingred.length;
  };

  const smoothieSentence = (itemOne, ...otherItems) => {
      if (itemOne && ingredCount === 0) {
          sentence += itemOne;
          ingredCount++;
      } else if (ingredCount > 0) {
          sentence += " and " + itemOne;
